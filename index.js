...
const config = {
    A187: 'BOT G GUNA', // TOBAT SU ASU
    instagram: 'https://instagram.com/aikawa_rei', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6289514343919', // INFO SU JNGAN DI UBAH
    youtube: 'https://t.me/Reii1', // KINTIL
    whatsapp: 'https://chat.whatsapp.com/aaaaaaa', // BABI
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Selamat!! beban group berkurang satu!!! @${num.split('@')[0]}\n Awokwokwok`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289514343919@s.whatsapp.net"] // ganti nomer lu
			...
			switch(command) {
              ...
              case 'tareksis':
              case 'tarik':
                    let err = fs.readFileSync('./src/tarekses.mp3')
                    client.sendMessage(from, err, MessageType.audio, { ptt: true, quoted: mek })
                    break
              case 'iri':
              case 'iri?':
              case 'iri bilang bos':
                    let eerr = fs.readFileSync('./src/iri.mp3')
                    client.sendMessage(from, eerr, MessageType.audio, { ptt: true, quoted: mek })
                    break
                    case 'welot':
              case 'yamette':
              case 'yamette kudasai':
                    let eerr = fs.readFileSync('./src/yamette.mp3')
                    client.sendMessage(from, eerr, MessageType.audio, { ptt: true, quoted: mek })
                    break
              case 'kangcopet':
              case 'copet':
                    let errr = fs.readFileSync('./src/welot.mp3')
                    client.sendMessage(from, errr, MessageType.audio, { ptt: true, quoted: mek })
                    break
              case 'jago':
              case 'abangjago':
              case 'abgjago':
                    let eerrr = fs.readFileSync('./src/abangjago.mp3')
                    client.sendMessage(from, eerrr, MessageType.audio, { ptt: true, quoted: mek })
                    break    
              case 'salam':
              case 'p':
                    let eeerrr = fs.readFileSync('./src/salam.mp3')
                    client.sendMessage(from, eeerrr, MessageType.audio, { ptt: true, quoted: mek })
                    break   
               case 'baka':
              case 'onidah':
                    let errrr = fs.readFileSync('./src/baka.mp3')
                    client.sendMessage(from, errrr, MessageType.audio, { ptt: true, quoted: mek })
                    break  
              case 'boong':
              case 'tapi':
                    let eerrrr = fs.readFileSync('./src/tapi.mp3')
                    client.sendMessage(from, eerrrr, MessageType.audio, { ptt: true, quoted: mek })
                    break      
                     
                     
                     case 'join': 
            if (args.length == 0) return reply(from, `Jika kalian ingin mengundang bot kegroup silahkan invite atau dengan\nketik ${prefix}join [link group]`)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await client.inviteInfo(linkgrup)
            if (!islink) return reply(from, 'Maaf link group-nya salah! silahkan kirim link yang benar')
            if (!isOwner) {
                await client.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await client.sendMessage(from, 'Berhasil join grup via link!',text, {quoted: mek })
                          await client.sendMessage(chekgrup.id, `Hai minna~, Im BOT G GUNA. To find out the commands on this bot type ${prefix}help`,text, {quoted: mek })
                      })
            } else {
                let cgrup = await client.getAllGroups()
                if (cgrup.length > groupLimit) return reply(from, `Sorry, the group on this bot is full\nMax Group is: ${groupLimit}`)
                if (cgrup.size < memberLimit) return reply(from, `Sorry, BOT wil not join if the group members do not exceed ${memberLimit} people`)
                await client.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await reply(from, 'Berhasil join grup via link!')
                      })
                      .catch(() => {
                          reply(from, 'Gagal!')
                      })
	    ...
	    if (!isOwner) return reply(mess.only.ownerB)
            ...
            reply(from, 'Success kick all member')
        break
        ...
        case 'ban':
          	if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
            if (args.length == 0) return reply(from, `Untuk banned seseorang agar tidak bisa menggunakan commands\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`)
            if (args[0] == 'add') {
            ...
            client.sendFile(from, sesPic, 'session.png', 'Nehh...', id)
            break	
				case 'gtts':	
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, '𝗱𝗶𝗽𝗲𝗿𝗹𝘂𝗸𝗮𝗻 𝗸𝗼𝗱𝗲 𝗯𝗮𝗵𝗮𝘀𝗮!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '𝗧𝗲𝗸𝘀 𝘆𝗮𝗻𝗴 𝗺𝗮𝘂 𝗱𝗶𝗷𝗮𝗱𝗶𝗶𝗻 𝘀𝘂𝗮𝗿𝗮 𝗺𝗮𝗻𝗮? 𝘁𝘆𝘁𝗱 𝗸𝗮𝗵?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
				...
				case 'ytsearch': 
					if (args.length < 1) return reply('𝘆𝗮𝗻𝗴 𝗺𝗮𝘂 𝗱𝗶𝗰𝗮𝗿𝗶 𝗮𝗽𝗮? 𝘁𝘆𝘁𝗱 𝗸𝗮𝗵?')
				...
				case 'lneon':
					...
					tels = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=neon_light&text1=${tels}&text2=${tels}`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
					break	
				case 'lion':
					...
					tels = body.slice(6)
					reply(mess.wait)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/textpro?theme=lionlogo&text1=${tels}&text2=${tels}`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
					break				
				case 'glitch':
...
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
            ...
            client.sendMessage(from, `*Link Group: ${groupMetadata.subject}*\n\n${isLinkGC.repeat(1)}`, text, { quoted: mek})
            break
            case 'adminbot':
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Nehhh.... Dia need loli....',MessageType.text, { quoted: mek} )
           break  
             case 'out':
             case 'keluar':
           case 'leave':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			 		client.sendMessage(from, '𝐘𝐞𝐚𝐡 𝐝𝐢 𝐬𝐮𝐫𝐮𝐡 𝐤𝐞𝐥𝐮𝐚𝐫 𝐬𝐚𝐦𝐚 𝐨𝐰𝐧𝐞𝐫,,𝐛𝐲𝐞𝐞𝐞 :)', text, {quoted: mek})
		...
		case 'del':
          	if (!isGroupAdmins) return reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!')
            	if (!isOwner) return reply(mess.only.ownerB)
            if (!quotedMsgObj.fromMe) return reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`)
